...
export const WorkspaceTab: React.FC<WorkspaceTabProps> = ({ snapshot }) => {
  const workspacePath = snapshot.verification?.workspacePath;
  ...
  return (
    <Box flexDirection="column" padding={1} width="100%">
      <Box borderStyle="single" borderColor="cyan" padding={1} flexDirection="column">
        <Text bold color="cyan">ABSOLUTE WORKSPACE PATH: {workspacePath || 'not available'}</Text>
        <Text color="gray">Metrics: {scanResult.fileCount} Files | {scanResult.totalLines} LOC | {(scanResult.totalBytes / 1024).toFixed(1)} KB</Text>
      </Box>
